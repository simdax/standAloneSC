TaskProxyGui : JITGui {
	...
	*observedClass { ^TaskProxy }
	...
	makeViews { |options|
		...
		var width = lineWidth * 0.8 / 7;
		var nameWidth = lineWidth * 0.18;
		...
		quantView=(
			//			refresh:{arg s; s.w.refresh},
			'value_':{arg s, quant;
				quant.asArray.do{arg x, i;
					s.w.children[i].value_(x)
				}; s.w
			},
			w:View(
				zone,Rect(0,0, width*3, height)
			).background_(Color.rand)
			.layout_(
				HLayout(
					*([\quant, \phase, \timingOffset]+++
						//random specs ?
						[[0,20], [-4, 4], [0, 2]])
					.collect{arg x;
						NumberBox()
						.action_{
							|source|
							var quant=object.quant;
							var res=source.value.min(x[2]).max(x[1]);
							if(object.respondsTo('align')){
								object.align(
									quant.asQuant.perform(
										(x[0]++'_').asSymbol,
										res
									)
								);
	...
	checkUpdate {
		...
		if (newState[\quant] != prevState[\quant]) {
			quantView.perform('value_', (newState[\quant])).refresh;
		};
...
TaskProxyAllGui :JITGui {
	...
	*new { |numItems = 16, parent, bounds, makeSkip = true, options = #[]|
	...
	makeViews { |options|
		...
		try{parent.view}
		{parent.addFlowLayout; parent}
		.decorator.left_(zone.bounds.right - 12)
	...
	*observedClass { ^Pdefn }
	...
	checkUpdate {
		...
		if (newState[\source] != prevState[\source]) {
			defer { csView.textField.string_(object); };
		};
...
PdefAllGui : TaskProxyAllGui {
	...
	*tpGuiClass { ^PdefGui }
	...
	setDefaults {
		defPos = 270@660;
		minSize = 360 @ (numItems + 1 * 20);
	}
}

PdefnAllGui : TaskProxyAllGui {
	*observedClass { ^Pdefn }
	*tpGuiClass { ^PdefnGuiPlus }
