...
PatternProxy : Pattern {
	...
	classvar <>defaultQuant=1.0, defaultEnvir;
...
Pdef : EventPatternProxy {
	...
	*new { arg key, item, track=true;
	...
	track{
		tracked=true;
		this.source=source.collect{
			arg x; var tmp=x.copy; tmp[\freq]=nil; tmp[\sustain]=nil;
			PdefCol(key).source=tmp;
			x
		};
	}
	play { arg argClock, protoEvent, quant, doReset=false;
		...
		if(player.isNil) {
			...
			// resets  when stream has endedd or after pause/cmd-period:
			if(player.streamHasEnded or: { player.wasStopped }) { doReset = true };
			protoEvent !? { player.event = protoEvent };
			if(player.isPlaying.not) {
				player.play(argClock, doReset, playQuant);
...
PdefCol : Pattern{
	...
	*new { arg key, item;
	...
	asStream{
		^Pfunc{this.source ? 1}.asStream
	}
	doesNotUnderstand{ arg msg;
		try{
			^Pfunc{this.source.at(msg)}.asStream
		}
		{Error("pdefCol marche pas !").throw}
	}
	untrack{
		Pdef(key).tracked=false
	}
}
